    ...
    @if (item) {
      <div class="custom-modal-overlay" (click)="closeOnBackdrop($event)">
        <div class="modal-dialog-custom card-luxury p-3 p-sm-4 p-md-5 overflow-y-auto" (click)="$event.stopPropagation()">
          ...
          <!-- Product Image -->
          <div class="main-image-wrapper rounded-3 overflow-hidden mb-3 mb-md-4">
            <img [src]="item.image" [alt]="item.name" class="img-fluid w-100 object-fit-cover">
          </div>

          <!-- Description -->
          <div class="mb-3 mb-md-4">
            <h5 class="text-orange-glow fw-bold mb-2 fs-6 fs-md-5">
              <i class="bi bi-info-circle me-2"></i>{{ t.isAmharic() ? 'የዕቃው ማብራሪያ' : 'Product Overview' }}
            </h5>
          ...
          <!-- Technical Specs Table (Responsive 1-col mobile, 2-col tablet/desktop) -->
